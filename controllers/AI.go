...
// GenerateContent は Gin のハンドラーとして修正されたバージョン
func (ctrl AIController) AddBrackets(c *gin.Context) {
	var aiForm forms.AddBracketsForm
	...
	fmt.Println("Step2")
	// バリデーション
	if aiForm.TargetSentence == "" || aiForm.TargetWord == "" {
		log.Printf("Missing prompt: %+v", aiForm)
		c.JSON(http.StatusBadRequest, gin.H{"message": "Prompt is required"})
		return
	}
	...
	// 質問を送信して回答を取得する
	model := client.GenerativeModel("gemini-pro")
	prompt := genai.Text(fmt.Sprintf("Please include '%s' with the brackets {{}} and regenerate the following sentence: '%s'. Please make sure not to change the case of the word in the bracket (lowercase vs uppercase).", aiForm.TargetWord, aiForm.TargetSentence))
	resp, err := model.GenerateContent(context.Background(), prompt)
	if err != nil {
		log.Printf("Error calling Gemini API: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": fmt.Sprintf("Failed to call Gemini API: %v", err)})
